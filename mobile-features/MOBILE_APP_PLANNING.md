...
| Conversation CRUD/group actions | Mostly: create/list/detail, get-or-create direct, rename group, delete group, add/kick/leave, make admin, nickname. Group creation enforces three unique participants including creator; backend promotes the first remaining participant when the final admin leaves/is removed. | Conversation list/detail/settings, role-aware actions, dialogs, cache updates, empty/error/loading states. | No generic rename/delete for direct conversations by design; no group avatar endpoint. Leave/delete return `ApiResponse<Void>` while other mutations return canonical `ConversationResponse`. |
...
| Realtime social notifications | Yes: notification queue and unread-count queue plus REST list/read/read-all. Notification Service no longer consumes `message-sent`; list/count/mark-all exclude legacy `MESSAGE` rows. | Realtime insert/dedupe, badge update, foreground presentation, target routing, cache reconciliation. | REST list is unpaginated. `MESSAGE` remains an internal compatibility enum only and is not a mobile social-notification type. |
...
Contract source-validation decisions, remediation work, and verification gates are recorded in `docs/architecture/WEB_MOBILE_CONTRACT_REMEDIATION_PLAN.md`. Feature implementation must use the exact enum and DTO transformations in `mobile-features/README.md` and the affected numbered specification rather than inferring fields from UI copy.

As of the 2026-07-13 audit, the known web/backend P0 notification contradiction is resolved and verified. Remaining gaps are explicitly feature-gated; they are not client implementation assumptions.
...
| `ExploreRepository` | Composes `GET /posts` with `GET /users/search?q=...&limit=12`; current post/hashtag search and top-ten trending ranking are client-side until a dedicated paginated discovery API exists. |
...
- Share opens a caption-only sheet using `{ caption }`. Backend currently creates shares as `PUBLIC`; do not show share privacy until that request contract exists. Shared-post unavailable states must remain renderable.
...
| `FRIEND_ACCEPT` | Actor profile using `actorId`; current `targetId` is a friendship id. |
...
Feature-sized behavior specifications live in `docs/architecture/mobile-features/`. Start from its `README.md` and follow the implementation order listed there. The realtime transport spike intentionally precedes chat message publishing even though its filename is `08-*`. Those files are authoritative for current-web behavior, mobile interactions, state/error handling, and feature handoff checklists.
...
1. Select the next document in the implementation order from `docs/architecture/mobile-features/README.md`.
