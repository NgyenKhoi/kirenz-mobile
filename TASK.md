...
- Updated: 2026-07-15.
- Current feature: Feature 06 - Conversations and Groups (`TN-02`).
...
- Next checkpoint: account-scoped conversation cache/offline state, draft-discard protection, then canonical group settings mutations and screens.
...
| 2 | TN-02 | Implement Feature 06 Conversations and Groups | In progress | Active slice owns `features/chat/**`; coordinated `app/router.dart` change is recorded for chat detail/search/group routes | First list/direct/group-create vertical slice passes; then group settings checklist and full feature review pass; no message/realtime success is simulated |
...
| 06 | Conversations, Groups | In progress | Typed canonical entities/repository; sorted list with previews/unread; direct search/get-or-create dedupe; validated group create; shared list/detail state and routes | Account-scoped cache/offline; group draft discard; complete settings mutations/role UI/reconciliation; widget/repository coverage; review and device/backend acceptance |
...
| 06 Conversations | Pass, 2026-07-16 | 4 focused model/controller tests; full suite 51/51 pass | Runnable backend/device flow pending | Pending until full Feature 06 implementation | Pending | In progress |
